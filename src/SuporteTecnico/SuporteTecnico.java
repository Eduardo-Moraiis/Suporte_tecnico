...
public class SuporteTecnico {
    ...
    private void preencherBanco() {
        this.bancoSolucoes = new HashMap<String, String>();
    ...
    public String pesquisaSolucao(HashSet<String> palavras) {
        for(String palavra : palavras){
            for(String chaveBranco : bancoSolucoes.keySet()) {
                if(palavra.equals(chaveBranco)) {
                    return bancoSolucoes.get(chaveBranco);
                }
            } // fim for(String chaveBranco : bancoSolucoes.keySet())
        } // fim for(String palavra : palavras)
        return pesquisaSolucaoAleatoria();
    }

    public String pesquisaSolucaoAleatoria() {

        return respostaPadrao.get(gerador.nextInt(respostaPadrao.size()-1));
    }
